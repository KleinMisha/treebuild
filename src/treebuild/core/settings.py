...
GLOBAL_STORE_PATH = GLOBAL_TREEBUILD_DIR / STORE_FILE_NAME
...
class TreeBuildSettings(BaseSettings):
    ...
    session_file: Path = GLOBAL_STORE_PATH
...
def resolve_session_file(level: SettingsLevel, start_search: Path) -> Path:
    return _resolve_treebuild_file(level, start_search, filename=STORE_FILE_NAME)
